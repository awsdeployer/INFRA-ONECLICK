...
# Install dependencies (root step)
sudo apt-get update -y
sudo apt-get install -y curl tar jq

# Make sure runner dir is owned by ubuntu
sudo chown -R ubuntu:ubuntu "$RUNNER_DIR"

# Download latest GitHub runner (as ubuntu user)
...
# Install and start service (sudo is allowed only here)
sudo ./svc.sh install
sudo ./svc.sh start
